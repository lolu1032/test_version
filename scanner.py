...
def write_status(ledger, marks):
    """Render a human-readable STATUS.md dashboard (committed each run).

    Open the repo on GitHub and this is the front-page view: equity, return,
    cash, every open position with its live unrealized PnL / SL / TP, and the
    most recent closed trades. No JSON or Notion needed.
    """
    eq = ledger.get("equity", ledger["cash"])
    ret = ledger.get("total_return_pct", 0.0)
    start = ledger.get("starting_cash", STARTING_CASH)
    updated = (ledger.get("updated_at") or _now())[:16].replace("T", " ")
    arrow = "🟢" if ret >= 0 else "🔴"

    L = []
    L.append("# 📊 Paper Trading — Live Status")
    L.append("")
    L.append(f"_Updated **{updated} UTC** · run #{ledger.get('runs', 0)} · "
             f"strategy `{STRATEGY}` · SL −{STOP_LOSS:.0%} / TP +{TAKE_PROFIT:.0%}_")
    L.append("")
    L.append("| | |")
    L.append("|---|---|")
    L.append(f"| **Equity** | {arrow} **${eq:,.2f}**  ({_pct(ret)}) |")
    L.append(f"| Starting cash | ${start:,.2f} |")
    L.append(f"| Idle cash | ${ledger['cash']:,.2f} |")
    L.append(f"| Open positions | {len(ledger['positions'])} / {MAX_POSITIONS} |")
    L.append(f"| Closed trades | {len(ledger.get('history', []))} |")
    L.append("")
    ...
    L.append("")
    if ledger["positions"]:
        L.append("| Symbol | Entry | Current | Unreal. PnL | Stop-loss | Take-profit |")
        L.append("|---|---|---|---|---|---|")
        for sym, p in ledger["positions"].items():
            cur = marks.get(sym, p["entry_price"])
            upnl = (cur - p["entry_price"]) / p["entry_price"] * 100 if p["entry_price"] else 0.0
            tag = "🟢" if upnl >= 0 else "🔴"
            L.append(f"| `{sym}` | {p['entry_price']:.6g} | {cur:.6g} | "
                     f"{tag} {_pct(upnl)} | {p['stop_loss']:.6g} | {p['take_profit']:.6g} |")
    else:
        L.append("_No open positions — all cash idle, waiting for a signal._")
    L.append("")

    # --- Recent closed trades (last 10, newest first) ---
    hist = ledger.get("history", [])
    L.append(f"## Recent closed trades ({len(hist)} total)")
    L.append("")
    if hist:
        L.append("| Closed | Symbol | Reason | PnL | Return |")
        L.append("|---|---|---|---|---|")
        for r in reversed(hist[-10:]):
            when = (r.get("closed_at") or r.get("ts") or "")[:16].replace("T", " ")
            tag = "🟢" if (r.get("pnl") or 0) >= 0 else "🔴"
            L.append(f"| {when} | `{r.get('symbol','')}` | {r.get('reason','')} | "
                     f"{tag} {r.get('pnl', 0):+.2f} | {_pct(r.get('return_pct', 0))} |")
    else:
        L.append("_None yet — positions close when they hit stop-loss or take-profit._")
    L.append("")
    L.append("---")
    L.append("> Paper trading (simulated, no real money). "
             "Auto-generated by `scanner.py` every 15 min. "
             "Source of truth: `ledger.json`.")
    L.append("")

    with open(os.getenv("STATUS_PATH", "STATUS.md"), "w") as f:
        f.write("\n".join(L))
